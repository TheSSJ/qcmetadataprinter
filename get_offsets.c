...
#define PRINT(PARAM_ID, table_ptr)                                                                  \
    ...
    if ((blob_pointer != oss_pointer) || (blob_size != oss_size)) fprintf(fil,"FIXME!!!\n");             \
    fprintf(fil, "BLOB: %s index=%d pointer=%d size=%d\n", #PARAM_ID, PARAM_ID, blob_pointer, blob_size); \
    fprintf(fil, "OSS : %s index=%d pointer=%d size=%d\n", #PARAM_ID, PARAM_ID, oss_pointer, oss_size);
...
int main() {
    ...
    FILE *fil;
    fil = fopen("/data/tmp/get_offset.log", "w");
